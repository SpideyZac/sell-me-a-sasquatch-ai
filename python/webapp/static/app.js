...
function renderFirstPlayerSelect(select, numPlayers) {
    ...
    select.innerHTML =
        ...
        Array.from(
            { length: numPlayers },
            (_, i) => `<option value="${i}">player_${i}</option>`
        ).join("");
    if (previous && parseInt(previous, 10) < numPlayers)
...
function initWatch() {
    ...
    document
        .getElementById("watch-step")
        .addEventListener("click", async () => {
            if (!watchGameId) return;
            const res = await fetch(`/api/games/${watchGameId}/advance`, {
                method: "POST",
            });
            const data = await res.json();
            renderWatchState(data.state);
    ...
    document
        .getElementById("watch-autoplay")
        .addEventListener("change", (e) => {
            if (e.target.checked) {
                watchAutoplayTimer = setInterval(async () => {
                    if (!watchGameId) return;
                    const res = await fetch(
                        `/api/games/${watchGameId}/advance`,
                        { method: "POST" }
                    );
                    const data = await res.json();
                    renderWatchState(data.state);
...
function renderWatchState(state) {
  ...
  `;
    ...
    document.getElementById("watch-log").innerHTML =
        "<h3>Log</h3>" + state.log.map((l) => `<div>${l}</div>`).join("");
}
...
function initPlay() {
    ...
    const refresh = () => {
        ...
        seatSel.innerHTML = Array.from(
            { length: n },
            (_, i) => `<option value="${i}">player_${i}</option>`
        ).join("");
        renderSeatModelSelects(
            seatModelsDiv,
            n,
            parseInt(seatSel.value || "0", 10)
        );
    ...
    document
        .getElementById("play-start")
        .addEventListener("click", async () => {
            const numPlayers = parseInt(numSel.value, 10);
            const humanSeat = parseInt(seatSel.value, 10);
            const seatModels = collectSeatModels(
                seatModelsDiv,
                numPlayers,
                humanSeat
            );
...
function renderPlayState(state) {
  ...
  `;
    ...
    document.getElementById("play-hand").innerHTML =
        `<h3>Your hand (${state.point_tokens[state.your_seat]} tokens)</h3>` +
        state.hand.map(cardLabel).join(", ");
    ...
    document.getElementById("play-players").innerHTML =
        ...
        state.collections
            .map(
                (c, p) =>
                    `<div class="player-card"><b>player_${p}</b> - ${state.point_tokens[p]} tokens - collection: ${c.map(cardLabel).join(", ") || "(empty)"}</div>`
            )
            .join("");
    ...
    if (state.your_turn && !state.is_game_over) {
        actionsDiv.innerHTML =
            ...
            state.legal_actions
                .map(
                    (a) =>
                        `<button class="action-btn" data-idx="${a.index}">${a.label}</button>`
                )
                .join("");
        actionsDiv
            ...
            .forEach((btn) =>
                btn.addEventListener("click", () =>
                    playAct(parseInt(btn.dataset.idx, 10))
                )
            );
...
function kindSelectHtml(kindOptions) {
    const opts = kindOptions
        .map(
            (k) =>
                `<option value="${k.value}" ${k.remaining <= 0 ? "disabled" : ""}>${k.label} (${k.remaining} left)</option>`
        )
        .join("");
...
function renderLivePrompt(state) {
    ...
    if (prompt.type === "pin_hand") {
        const heading =
            prompt.context === "starting_hand"
                ? "Enter your starting hand"
                : "What did you draw?";
        promptDiv.innerHTML = `
      <h3>${heading}</h3>
    ...
    `;
        document
            ...
            .addEventListener("click", () => {
                liveRespond({
                    kinds: Array.from(
                        promptDiv.querySelectorAll(".kind-select")
                    ).map((s) => s.value),
                });
            });
        return;
    ...
    if (prompt.type === "reveal_kind") {
        ...
        document
            ...
            .addEventListener("click", () => {
                liveRespond({
                    kind: promptDiv.querySelector(".kind-select").value,
                });
            });
        return;
    }
    ...
    if (prompt.type === "choose_action") {
        const heading = prompt.your_turn
            ? "Your move"
            : `player_${prompt.seat}'s move <small>(enter what actually happened)</small>`;
        promptDiv.innerHTML =
            `<h3>${heading}</h3>` +
            prompt.options.map(liveActionButtonHtml).join("");
        promptDiv
            .querySelectorAll(".action-btn")
            .forEach((btn) =>
                btn.addEventListener("click", () =>
                    liveRespond({ index: parseInt(btn.dataset.idx, 10) })
                )
            );
        return;
    }
...
function renderLiveState(state) {
  ...
  `;
    ...
    document.getElementById("advisor-hand").innerHTML =
        `<h3>Your hand (${state.point_tokens[state.your_seat]} tokens)</h3>` +
        (state.hand.map(cardLabel).join(", ") || "(empty)");
    ...
    document.getElementById("advisor-log").innerHTML =
        "<h3>Log</h3>" + state.log.map((l) => `<div>${l}</div>`).join("");
}
