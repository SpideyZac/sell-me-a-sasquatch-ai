...
from stable_baselines3.common.callbacks import BaseCallback  # type: ignore  # noqa: E402
from stable_baselines3.common.monitor import Monitor  # type: ignore  # noqa: E402
from stable_baselines3.common.vec_env import DummyVecEnv, SubprocVecEnv  # type: ignore  # noqa: E402  # pylint: disable=line-too-long
...
def make_env(players, deck_path, use_pool, current_prob, max_snapshots, seed):
    ...
    opponent = (
        OpponentPool(current_prob=current_prob, max_snapshots=max_snapshots)
        if use_pool
        else random_masked_policy
    )
    env = SasquatchSelfPlayEnv(
        players=players, deck_config_path=deck_path, opponent_policy=opponent
    )
    env.reset(seed=seed)
    env = ActionMasker(env, lambda e: e.action_masks())  # type: ignore
    return Monitor(env)
...
class SnapshotCallback(BaseCallback):
    ...
    def __init__(
        ...
        save_dir: str,
        every_n_steps: int,
        in_process_pool: OpponentPool | None = None,
        verbose: int = 0,
    ):
    ...
    def _on_step(self) -> bool:
        ...
        if self.verbose:
            print(
                f"[snapshot] {self._count} opponent snapshot(s) published at {self.num_timesteps} timesteps"  # pylint: disable=line-too-long
            )
        return True
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[2, 3, 4, 5, 6],
        choices=range(2, 7),
        help="Table sizes to train across; one is drawn per episode. The default trains a single general model.",  # pylint: disable=line-too-long
    )
    ...
    parser.add_argument(
        ...
        default="subproc",
        help="'dummy' keeps every env in this process",
    )
    ...
    parser.add_argument(
        ...
        choices=["self", "random"],
        default="self",
        help="'self' = a league of the policy's own frozen snapshots (default); 'random' = a random baseline only, much weaker",  # pylint: disable=line-too-long
    )
    parser.add_argument(
        "--snapshot-every",
        type=int,
        default=50_000,
        help="Timesteps between opponent-league snapshots",
    )
    parser.add_argument(
        "--current-prob",
        type=float,
    ...
    parser.add_argument(
        "--max-snapshots", type=int, default=8, help="Opponent league size per worker"
    )
    parser.add_argument(
        "--n-steps",
        type=int,
        default=512,
        help="Rollout length per env between updates",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Entropy bonus; keeps the policy exploring the wide action space",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Width of the per-action embedding in the pointer head",
    )
...
def main(argv=None):
    ...
    vec_env = (
        DummyVecEnv(env_fns)  # type: ignore
        if in_process
        else SubprocVecEnv(env_fns, start_method="spawn")  # type: ignore
    )
    ...
    if args.resume:
        ...
        model = MaskablePPO(
            ...
            tensorboard_log=args.tensorboard_log,
            policy_kwargs=dict(
                net_arch=list(args.net_arch), action_embed_dim=args.action_embed_dim
            ),
        )
    ...
    if use_pool:
        ...
        if in_process:
            ...
            if existing:
                print(
                    f"Reloaded {min(len(existing), args.max_snapshots)} opponent snapshot(s) from {snapshot_dir}"  # pylint: disable=line-too-long
                )
        callback = SnapshotCallback(
            snapshot_dir, args.snapshot_every, in_process_pool=live_pool, verbose=1
        )

    print(
        f"Training one policy across {players} players on {args.n_envs} {'in-process' if in_process else 'worker'} env(s); "  # pylint: disable=line-too-long
        f"action space width {vec_env.action_space.n}"  # type: ignore
    )
    ...
    model.save(args.out)
    print(
        f"Saved model to {args.out}.zip (total lifetime timesteps: {model.num_timesteps})"
    )
    vec_env.close()
