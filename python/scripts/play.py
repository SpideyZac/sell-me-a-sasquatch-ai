...
def as_opponent(model, rng: np.random.Generator):
    ...
    try:
        ...
        def _torch_opponent(obs, mask, _legal_count):
...
def evaluate(
    model, num_players: int, episodes: int, deck, opponent, seed: int
) -> tuple[float, float]:
...
def main(argv=None):
    ...
    parser.add_argument(
        "--players", type=int, nargs="+", default=[2, 3, 4, 5, 6], choices=range(2, 7)
    )
    ...
    opponent = (
        random_masked_policy if args.opponent == "random" else as_opponent(model, rng)
    )

    print(
        f"{args.model_path} vs '{args.opponent}' opponents, {args.episodes} episodes per table size\n"  # pylint: disable=line-too-long
    )
    print(
        f"{'players':>8} {'win rate':>10} {'chance':>8} {'vs chance':>10} {'episodes/s':>11}"
    )
    for num_players in sorted(set(args.players)):
        win_rate, rate = evaluate(
            model, num_players, args.episodes, deck, opponent, args.seed
        )
        chance = 1.0 / num_players
        print(
            f"{num_players:>8} {win_rate:>9.1%} {chance:>7.1%} {win_rate / chance:>9.2f}x {rate:>10.1f}"  # pylint: disable=line-too-long
        )
