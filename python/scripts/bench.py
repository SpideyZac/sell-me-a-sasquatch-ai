...
from sell_me_a_sasquatch import _native as native  # type: ignore  # noqa: E402
...
def bench_env(players, seconds: float, deck) -> tuple[float, float]:
    ...
    env = SasquatchSelfPlayEnv(
        players=players, deck_config_path=deck, opponent_policy=random_masked_policy
    )
    env.reset(seed=0)
...
def bench_engine(num_players: int, seconds: float, deck) -> float:
    ...
    actions = np.zeros(
        (max_actions, sasquatch_spaces.ACTION_FEAT_LEN), dtype=np.float32
    )
    game = native.Game(num_players, deck, 1)  # pylint: disable=c-extension-no-member
    steps = 0
    deadline = time.perf_counter() + seconds
    while time.perf_counter() < deadline:
...
def main(argv=None):
    ...
    parser.add_argument(
        "--engine", action="store_true", help="Also report the raw Rust-side rate"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=1,
        help="Run this many processes and sum their rates",
    )
    ...
    configs: list = list(sasquatch_spaces.ALL_PLAYER_COUNTS) + [  # type: ignore
        sasquatch_spaces.ALL_PLAYER_COUNTS
    ]
    ...
    for players in configs:
        label = (
            "mixed" if isinstance(players, tuple) and len(players) > 1 else str(players)
        )
        if pool is None:
            steps, episodes = bench_env(players, args.seconds, deck)
        else:
            results = pool.map(
                _worker, [(players, args.seconds, args.deck)] * args.workers
            )
