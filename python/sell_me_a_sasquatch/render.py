...
def render_state(game, observer: int | None = None) -> str:
    ...
    lines = []
    lines.append(
        f"phase={game.current_phase()} turn_leader={game.turn_leader()} active={game.active_players()}"
    )
    ...
    if observer is None:
        ...
        lines.append(
            f"player_{observer} hand: {', '.join(_card_label(game, c) for c in obs.own_hand)}"
        )

    lines.append(
        "point tokens: "
        + ", ".join(f"p{p}={t}" for p, t in enumerate(obs.point_tokens))
    )
    lines.append(
        "collection sizes: "
        + ", ".join(f"p{p}={len(c)}" for p, c in enumerate(obs.collections))
    )
    ...
    if obs.deals:
        ...
        for deal in obs.deals:
            ...
            lines.append(
                f"  seller=player_{deal.seller} revealed=[{revealed}] hidden={deal.num_hidden}"
            )
    else:
