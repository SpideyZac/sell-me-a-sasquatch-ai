...
/// §2.3 steps 1-5: deal offer, seller reveal, Buyer's extra peek, empty
/// Thingamabob window, Buyer's final commit - full sequencing, no shortcuts.
#[test]
fn deal_offer_reveal_peek_and_choose_sequencing() {
    ...
    while game.current_phase().starts_with("deal_offer") {
        ...
        assert_ne!(player, buyer, "the Buyer never submits a deal in Buyer mode");
    ...
    assert_eq!(peek_targets.len(), 3, "buyer may peek into any of the 3 sellers' deals");
    let Action::BuyerPeek { target_seller } = peek_targets[0] else { panic!() };
    let events = game.apply_action(buyer, Action::BuyerPeek { target_seller }).unwrap();
...
/// §2.3 step 5: chosen seller's cards go to the (old) Buyer's Collection;
/// every other seller keeps their own 3 cards in their own Collection; the
...
fn choosing_a_deal_distributes_collections_and_passes_marker() {
    ...
    let Action::BuyerPeek { target_seller: _ } = target else { panic!() };
    game.apply_action(old_buyer, target).unwrap();
    auto_pass_thingamabob_window(&mut game);

    let Action::ChooseDeal { seller: chosen } = game.legal_actions(old_buyer)[0].clone() else { panic!() };
    let other_seller = (0..3).find(|&p| p != old_buyer && p != chosen).unwrap();

    let events = game.apply_action(old_buyer, Action::ChooseDeal { seller: chosen }).unwrap();

    assert!(events.iter().any(|e| matches!(e, Event::TurnLeaderPassed { new_leader } if *new_leader == chosen)));
    assert_eq!(game.turn_leader(), chosen, "marker passes to the chosen seller");
...
/// Flagged load-bearing interpretation (§2.4 note): the Buyer marker has
/// already passed to the new Buyer *before* trade-in/penalty resolution
/// runs, so it's the new Buyer (not the one who just finished their turn)
/// who resolves Nasty penalties.
#[test]
fn nasty_penalty_resolver_is_the_new_buyer_not_the_old_one() {
    ...
    while game.current_phase().starts_with("deal_offer") {
        ...
        if game.current_phase() == "deal_offer_submit" {
            let ppb = hand_cards_of_kind(&game, player, sasquatch_engine::card::CardKind::Nasty(sasquatch_engine::card::NastyKind::PoisonPillBug));
    ...
    auto_pass_thingamabob_window(&mut game);
    let Action::ChooseDeal { seller: chosen } = game.legal_actions(old_buyer)[0].clone() else { panic!() };
    game.apply_action(old_buyer, Action::ChooseDeal { seller: chosen }).unwrap();
