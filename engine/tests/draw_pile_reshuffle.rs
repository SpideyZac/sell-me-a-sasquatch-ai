...
fn draw_pile_reshuffles_from_discard_when_exhausted() {
    ...
    // Turn 1 (§2.7): the active player's 3-card split-deal removes exactly
    // 3 cards from *their own* hand, so refilling them exactly drains the
    // 3-card draw pile - no reshuffle needed yet.
    auto_play_deal_offers(&mut game);
    auto_pass_thingamabob_window(&mut game);
    game.apply_action(second, Action::RespondToDeal { reverse: false }).unwrap();
    assert_eq!(game.turn_leader(), second, "active player alternates every turn");
    ...
    auto_play_deal_offers(&mut game);
    auto_pass_thingamabob_window(&mut game);
    let events = game.apply_action(first, Action::RespondToDeal { reverse: false }).unwrap();
