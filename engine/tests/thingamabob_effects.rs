...
use sasquatch_engine::{
    ...
    card::{CardKind, ThingamabobKind, Tier},
    game::Event,
};
...
fn platonic_isolator_steals_one_token_from_a_strictly_richer_player() {
    for seed in 0..40u64 {
        ...
        assert_eq!(
            game.player_point_tokens(richer),
            1,
            "a lone Giant is a complete set worth 1 token"
        );

        let Some(actor) = stash_cards_of_kind_in_collection(
            &mut game,
            CardKind::Thingamabob(ThingamabobKind::PlatonicIsolator),
            1,
        ) else {
            continue;
        };
        ...
        let Some(action) = find_thingamabob_action(
            &game,
            actor,
            |p| matches!(p, ThingamabobParams::PlatonicIsolator { target_player } if *target_player == richer),
        ) else {
            continue;
        };
...
fn detrital_repositioner_removes_one_card_from_a_deal_to_discard() {
    for seed in 0..40u64 {
        ...
        let Some(actor) = stash_cards_of_kind_in_collection(
            &mut game,
            CardKind::Thingamabob(ThingamabobKind::DetritalRepositioner),
            1,
        ) else {
            continue;
        };
        ...
        let Some(action) = find_thingamabob_action(
            &game,
            actor,
            |p| matches!(p, ThingamabobParams::RemoveFromDeals { removals } if removals.len() == 1),
        ) else {
            continue;
        };
        let Action::PlayThingamabob {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, Event::CardsDiscarded { cards } if cards == &vec![target_card])));
        let _ = target_seller;
        return;
    }
...
fn super_detrital_repositioner_allows_zero_one_or_two_removals_across_up_to_two_deals() {
    for seed in 0..40u64 {
        ...
        let Some(actor) = stash_cards_of_kind_in_collection(
            &mut game,
            CardKind::Thingamabob(ThingamabobKind::SuperDetritalRepositioner),
            1,
        ) else {
            continue;
        };
        ...
        assert!(
            options.contains(&0),
            "playing the card with 0 removals (just to discard it) must be legal"
        );
        assert!(
            options.iter().all(|&n| n <= 2),
            "at most 2 cards may be removed"
        );
...
fn cryptozootic_expander_moves_a_hidden_hand_card_into_a_deal() {
    for seed in 0..40u64 {
        ...
        let Some(actor) = stash_cards_of_kind_in_collection(
            &mut game,
            CardKind::Thingamabob(ThingamabobKind::CryptozooticExpander),
            1,
        ) else {
            continue;
        };
        ...
        advance_thingamabob_window_to_player(&mut game, actor);
        let Some(action) = find_thingamabob_action(&game, actor, |_| true) else {
            continue;
        };
        let Action::PlayThingamabob {
            params: ThingamabobParams::CryptozooticExpander { target_deal, .. },
            ..
        } = action.clone()
        else {
            continue;
        ...
        game.apply_action(actor, action).unwrap();
        assert_eq!(
            game.player_hand(actor).len(),
            hand_before - 1,
            "the hand card moved out of hand"
        );
        let _ = target_deal;
        return;
    }
...
fn spectroelectric_optimeter_reveals_a_hidden_deal_card() {
    for seed in 0..40u64 {
        ...
        let Some(actor) = stash_cards_of_kind_in_collection(
            &mut game,
            CardKind::Thingamabob(ThingamabobKind::SpectroelectricOptimeter),
            1,
        ) else {
            continue;
        };
