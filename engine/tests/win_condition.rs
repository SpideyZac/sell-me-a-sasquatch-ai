//! §2.6: win threshold + tie handling ("first to reach the threshold AND
//! strictly more tokens than every other player, checked at each
//! end-of-turn").
...
fn win_is_declared_exactly_when_a_player_meets_threshold_and_strictly_leads() {
    ...
    for _ in 0..60 {
        ...
        let Some(_) = stash_cards_of_kind_in_collection(&mut game, CardKind::Creature(Tier::Giant), 1) else { continue };

        let tokens: Vec<u32> = (0..4).map(|p| game.player_point_tokens(p)).collect();
        let mut order: Vec<usize> = (0..4).collect();
        order.sort_by_key(|&p| std::cmp::Reverse(tokens[p]));
        let expected_winner = if tokens[order[0]] >= threshold && tokens[order[0]] > tokens[order[1]] { Some(order[0]) } else { None };

        assert_eq!(game.winner(), expected_winner, "tokens={tokens:?} threshold={threshold}");
...
fn tie_at_or_above_threshold_does_not_end_the_game() {
    ...
    for _ in 0..60 {
        ...
        if max >= threshold && leaders > 1 {
            assert!(!game.is_game_over(), "tied leaders at/above threshold must not end the game");
        }
    }
}
