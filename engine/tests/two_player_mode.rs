//! The materially different two-player flow. Confirmed against the
//! physical rulebook: on your turn you split exactly three of your own
//! hand cards between "my pile" and "their pile" (any split summing to
//! three: 3/0, 2/1, 1/2, 0/3), flip exactly one of those three cards face
//! up yourself, and the other player then accepts (each pile goes where
//! it was placed) or reverses (the two piles swap owners). Nasty-set
//! trade-ins always let the other player (not whoever's set completed)
//! decide what's taken.
...
fn only_the_active_player_acts_during_the_deal_split_and_reveal() {
    ...
    let Action::TwoPlayerSubmitDeal {
        own_pile,
        other_pile,
    } = game.legal_actions(active).into_iter().next().unwrap()
    else {
        panic!()
    };
    assert_eq!(
        own_pile.len() + other_pile.len(),
        3,
        "the split must always total 3 cards"
    );
    game.apply_action(
        active,
    ...
    assert_eq!(
        game.active_players(),
        vec![active],
        "only the active player reveals - never the responder"
    );
    ...
    assert_eq!(
        game.active_players(),
        vec![active],
        "Thingamabob window starts with the active player"
    );
    assert_eq!(
        game.legal_actions(responder),
        vec![],
        "the responder never gets a deal-offer action of their own"
    );
}
...
fn every_split_size_is_offered_as_a_legal_action() {
    ...
    let active = game.turn_leader();
    let mut sizes: Vec<(usize, usize)> = game
...
fn all_3_cards_come_from_the_active_players_own_hand() {
    ...
    for c in &offered {
        assert!(
            active_hand_before.contains(c),
            "every offered card must come from the active player's own hand"
        );
    }
    ...
    .unwrap();
    assert_eq!(
        game.player_hand(responder),
        responder_hand_before.as_slice(),
        "the responder's hand is never touched by the split"
    );
}
...
fn accept_sends_each_pile_where_it_was_placed() {
    ...
    game.apply_action(
        active,
        Action::TwoPlayerSubmitDeal {
            own_pile: own_pile.clone(),
            other_pile: other_pile.clone(),
        },
    )
    .unwrap();
    let reveal = game.legal_actions(active).into_iter().next().unwrap();
    ...
    let events = game
        .apply_action(responder, Action::RespondToDeal { reverse: false })
        .unwrap();
    let awarded_to_active = events.iter().find_map(|e| {
        if let Event::CardsAwarded { player, cards } = e {
            (*player == active).then(|| cards.clone())
        } else {
            None
        }
    });
    let awarded_to_responder = events.iter().find_map(|e| {
        if let Event::CardsAwarded { player, cards } = e {
            (*player == responder).then(|| cards.clone())
        } else {
            None
        }
    });
    assert_eq!(
        awarded_to_active.map(|mut c| {
            c.sort_unstable();
            c
        }),
        Some({
    ...
    assert_eq!(
        awarded_to_responder.map(|mut c| {
            c.sort_unstable();
            c
        }),
        Some({
            let mut v = other_pile;
            v.sort_unstable();
            v
        })
    );
...
fn reverse_swaps_the_two_piles_between_players() {
    ...
    game.apply_action(active, reveal).unwrap();
    auto_pass_thingamabob_window(&mut game);
    game.apply_action(responder, Action::RespondToDeal { reverse: true })
        .unwrap();

    for c in own_pile {
        assert!(
            game.player_collection(responder).contains(&c) || is_discarded_via_trade_in(&game, c),
            "active's own-pile card should land with the responder on reverse"
        );
    }
    for c in other_pile {
        assert!(
            game.player_collection(active).contains(&c) || is_discarded_via_trade_in(&game, c),
            "active's other-pile card should land with the active player on reverse"
        );
    }
}

fn is_discarded_via_trade_in(
    game: &sasquatch_engine::game::GameState,
    card: sasquatch_engine::card::CardId,
) -> bool {
